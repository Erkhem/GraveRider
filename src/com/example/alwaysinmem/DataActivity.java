...
public class DataActivity extends ListActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		graves = new ArrayList<Grave>(); // gson.fromJson(fileContent,
											// listType);
		 ...
		 * rowToAdd.setLayoutParams(layMoutParams);
		 * 
		 ...
		 * navBtn.setLayoutParams(layoutParams); navBtn.setText("Nawiguj");
		 * 
		 ...
		 * shareBtn.setLayoutParams(layoutParams); shareBtn.setText("Dziel");
		 * 
		 * shareBtn.setOnClickListener(new View.OnClickListener() {
		 * 
		 * @Override public void onClick(View v) { Intent dataIntent = new
		 * Intent(DataActivity.this, ShareActivity.class);
		 * dataIntent.putExtra(GRAVE_BUNDLE, grave); startActivity(dataIntent);
		 * } });
		 * 
		 * sendBtn.setOnClickListener(new View.OnClickListener() {
		 * 
		 * @Override public void onClick(View v) { restUtils.send(grave); } });
		 * 
		 * navBtn.setOnClickListener(new View.OnClickListener() {
		 * 
		 * @Override public void onClick(View v) { if (grave.getLattitude() !=
		 * null && grave.getLongtitude() != null) { Uri gmmIntentUri = Uri
		 * .parse("google.navigation:q=" + grave.getLattitude() + "," +
		 * grave.getLongtitude()); Intent mapIntent = new
		 * Intent(Intent.ACTION_VIEW, gmmIntentUri);
		 * mapIntent.setPackage("com.google.android.apps.maps");
		 * startActivity(mapIntent); } } });
		 * 
		 * rowToAdd.addView(nameLbl); rowToAdd.addView(navBtn); if (isConnected)
		 * { rowToAdd.addView(sendBtn); rowToAdd.addView(shareBtn); }
		 * 
		 * table.addView(rowToAdd, new
	...
	public boolean onContextItemSelected(MenuItem item) {
		...
		case R.id.menu_delete:
			...
			try {
				restUtils.deleteByObject(graves.get(menu.position).getId());
		...
		case R.id.menu_modify:
			Intent mainIntent = new Intent(DataActivity.this, MainActivity.class);
			
			Grave grave = graves.get(menu.position);
			
			mainIntent.putExtra(GRAVE_BUNDLE, grave);
			mainIntent.putExtra(LoginActivity.CREDENDIALS, login);
			startActivity(mainIntent);
			Toast.makeText(getBaseContext(), "MODIFY", Toast.LENGTH_SHORT).show();
			break;
		case R.id.menu_navigateTo:
			navigate((Grave) getListView().getItemAtPosition(menu.position));
			break;
		case R.id.menu_share:
			shareInfoAboutGrave((Grave) getListView().getItemAtPosition(menu.position));
			break;
		}
	...
	private void navigate(Grave grave) {
		if (grave.getLattitude() != null && grave.getLongtitude() != null) {
			Uri gmmIntentUri = Uri.parse("google.navigation:q=" + grave.getLattitude() + "," + grave.getLongtitude());
